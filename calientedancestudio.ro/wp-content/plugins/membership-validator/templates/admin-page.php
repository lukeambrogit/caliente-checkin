<?php
/**
 * Legacy admin template (read-only).
 *
 * @deprecated Active Schedule Manager admin UI is rendered from:
 *             includes/addons/schedule-manager/templates/admin/schedule-list.php
 *
 * Keep this file only for backward compatibility with older custom includes.
...
// sunt definite în class-oc-admin.php în metoda admin_page()
?>
                            ...
                            if (!empty($product_attributes) && is_array($product_attributes)) {
                                foreach ($product_attributes as $product_id => $product_data) : ?>
                                    <option value="<?php echo esc_attr($product_id); ?>" 
                                        ...
                                        <?php echo esc_html($product_data['name']); ?> (ID: <?php echo intval($product_id); ?>)
                                    </option>
                                <?php endforeach; 
                            }
                            ?>
...
window.renderExistingSchedule = function(scheduleData) {
    ...
    const weekdayRoNames = {
        ...
        4: 'joi', 5: 'vineri', 6: 'sambata', 7: 'duminica'
    };
...
window.populateFormWithAllScheduleData = function(scheduleData) {
    ...
    sortedWeekdays.forEach(function(weekdayNum) {
        ...
        // Set the weekday
        daySection.find('.oc-day-select').val(weekdayName);
        ...
        Object.keys(timeSlots).forEach(function(timeKey, timeIndex) {
            ...
            const timeValue = timeData.time.substring(0, 5); // Remove seconds (HH:MM:SS -> HH:MM)
            timeSelect.val(timeValue);
            // Clean(`Set time to ${timeValue} (from ${timeData.time}), current value:`, timeSelect.val());
